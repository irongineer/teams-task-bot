...
describe('UserId', () => {
  ...
  describe('fromString()', () => {
    ...
    it('無効な文字列を渡すと InvalidArgumentError を投げる', () => {
      expect(() => UserId.fromString('not-a-uuid')).toThrow(InvalidArgumentError);
    });

    it('空文字列を渡すと InvalidArgumentError を投げる', () => {
      expect(() => UserId.fromString('')).toThrow(InvalidArgumentError);
    });

    it('エラーメッセージに無効な値が含まれる', () => {
      expect(() => UserId.fromString('bad')).toThrow(
        'Invalid UserId: "bad" is not a valid UUID',
      );
    });
  });
